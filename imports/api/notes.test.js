...
if (Meteor.isServer) {
    describe('notes', function(){
        ...
        it('should not update note if user was not the creator', function() {
            const title = 'This is an updated title';
            Meteor.server.method_handlers['notes.update'].apply({
                userId: 'testid' // creator user's ID is noteOne.userId
            }, [
                noteOne._id,
                { title }
            ]);
        ...
        it('should return a users notes', function() {
            ...
            const notes = res.fetch(); // fetching from the cursor
            expect(notes.length).toBe(1);
            expect(notes[0]).toEqual(noteOne);
        });

        it('should return zero notes for user that has none', function() {
            const res = Meteor.server.publish_handlers.notes.apply({ userId: 'random' });
